...
*   **Kubernetes (K8s) Orchestration**: Migration from basic Docker Compose (which lacks robust auto-healing) to a fully declarative Kubernetes cluster. This will feature automatic Horizontal Pod Autoscaling (HPA) based on CPU/Memory utilization, self-healing node replacements, and zero-downtime Canary rolling updates.
