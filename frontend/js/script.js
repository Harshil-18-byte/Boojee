...
window.saveCart = () => {
    ...
    if (localStorage.getItem('boojee_logged_in') === 'true') {
        fetch('/api/cart', {
            method: 'POST',
...
window.syncCartFromBackend = async () => {
    if (localStorage.getItem('boojee_logged_in') !== 'true') return;
    try {
...
fetch('/api/products').then(res => res.json()).then(data => {
    if(data.products) {
        data.products.forEach(p => { backendProducts[p.name] = p.price; });
    }
...
document.querySelector('.desk-checkout').addEventListener('click', async () => {
    ...
    if (localStorage.getItem('boojee_logged_in') !== 'true') { notify('Please log in to proceed to checkout.'); setTimeout(() => location.href='login.html', 2000); return; }
